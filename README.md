...
## 심상세계 자유 선택 UI v0.1
- 심상세계 진입 시 `innerPhase` 기본값을 `menu`로 바꿔 강제 순서 대신 선택식으로 진행합니다.
- 메뉴 버튼: `정비`, `+5레벨`, `보상`, `스탯`, `아이템`, `상점`, `다음 층`
- `정비`/`+5레벨`/`보상 확정`은 심상세계 1회 진입당 1회만 실행되도록 제한합니다.

## 모바일 UI 간소화 v0.1
- 컨트롤 버튼 텍스트를 짧게 정리했습니다: `←`, `→`, `점프`, `대시`, `공격`, `스킬`, `마법`.
- HUD와 phasePanel 문구를 짧게 정리해 모바일에서 핵심 정보(상태, 층, 레벨, HP/MP, 적 정보, 코인, 인벤 개수) 위주로 표시합니다.
- `appendPanelControls()` 공통 부착을 유지해 저장 버튼 + 테스트 버튼이 모든 주요 화면에서 함께 보이도록 유지합니다.
