...
## v0.1 안정화 2차 확인 항목
- 다음 층 진입 시 HP/MP가 최대치로 회복되는지 확인
- 다음 층 진입 시 보상 선택 상태(`rewardCandidates`, `rewardSelected`, `rewardMeta`)가 초기화되는지 확인
- 패배 후 `처음부터 다시 시작`으로 전투 상태를 정상 복구할 수 있는지 확인
- `runDebugTests()` 신규 키 확인
...
- 모바일에서 조작이 꼬였을 때 새로고침 전 `패배 → 처음부터 다시 시작`으로 런 상태를 복구할 수 있음
